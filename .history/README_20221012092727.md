...
-  If you want to use ```xpath``` then use dev-tools, right click on element, go to copy, and slelect ```copy xpath`` place it in the response to target the element.```  ```response.xpath('//')```
-